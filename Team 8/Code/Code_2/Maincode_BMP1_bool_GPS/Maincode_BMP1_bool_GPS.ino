...
//Pre-Setup of GPS module
    ...
    long period;    // period of sending data from GPS 
...
bool GPS()
{
  ...
  if (Serial.read()== '$')
  {
    ...
    if (MID[2]=='G' && MID[3]== 'G' && MID[4]== 'A')
    {
      ...
      if(PosFix[0] == '1') 
      {
        ...
        Serial.print(NSInd);    Serial.print(",");
        ...
        Serial.print(EWInd);    Serial.print(",");
  ...
  else if(millis() - period > 1200)     // period = 1000 milliseconds , made 1200 milliseconds for allowance.
  {
    Serial.println("!! GPS is not connected !!");      // when gps in not connected'
    period = millis();
    return true;
  }
    return false;
...
void MPU() 
{
  ...
  {         
    ...
    Serial.print(temperature/340.0 + 36.53);    Serial.println(",");    
    //Temperature is in degree celesius
